...
function showData(destinations) {
    const destinationsString = destinations.map(destination => {
    return /* html */ `
        <li>
            <figure>
                <img src="img/${destination.image}" alt="${destination.subtitle}">
                <figcaption>
                    <button class="favorite heart__btn" data-id="${destination.id}">
                        <svg class="heart__icon" xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12" fill="none">
                    ...
                    <a href="detaljer.html?id=${destination.id}">MORE</a>
                </figcaption>
            </figure>
        </li> `
    }).join("")
    const headerString = `<header class="hero"><h1 class="hero__title">Apartments for rent</></header>`
    mainWrapperDom.insertAdjacentHTML("beforeend", headerString);
    const mainDom = document.createElement("main");
    mainWrapperDom.insertAdjacentElement("beforeend", mainDom);
